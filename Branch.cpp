...
bool Branch::CanManageBranch(int managerId)
{
    ...
    for (const auto &branch : GetAllBranches())
    {
        ...
        {
            cerr << "Manager is already assigned as a manager to another branch\n";
            return false;
        }
    }
...
void Branch::AddBranch(const string &location, int managerId)
{
    // Check if the manager can manage another branch
    if (!CanManageBranch(managerId))
    {
        cerr << "Manager cannot manage more branches\n";
        return;
    }
    ...
    if (sqlite3_prepare_v2(db, "INSERT INTO Branches (Location, ManagerId) VALUES (?, ?)", -1, &statement, nullptr) != SQLITE_OK)
    {
    ...
    sqlite3_bind_int(statement, 2, managerId);
    ...
    // Get the branchId of the newly inserted branch
    int branchId = sqlite3_last_insert_rowid(db);

    // Finalize statement and close database
    sqlite3_finalize(statement);
    sqlite3_close(db);

    cout << "Branch added successfully with id: " << branchId << "\n";
}
