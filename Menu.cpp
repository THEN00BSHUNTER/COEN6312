...
void Menu::RunRegistrationMenu()
{
    ...
    while (true)
    {
        ...
        { // Register a Customer
            ...
            {
                cout << "Customer registered successfully with id: " << newCustomer->GetId() << "\n";
            }
            else
            {
        ...
        { // Register an Employee
            ...
            {
                cout << "Employee registered successfully with id: " << newEmployee->GetId() << "\n";
            }
            else
            {
        ...
        { // Register a Manager
            ...
            {
                cout << "Manager registered successfully with id: " << newManager->GetId() << "\n";
            }
            else
            {
        ...
        { // Register a Supplier
            ...
            {
                cout << "Supplier registered successfully with id: " << newSupplier->GetId() << "\n";
            }
            else
            {
          ...
          // Get the logged-in username
            ...
            if (userId != -1)
            {
                ...
                do
                {
                    ...
                    if ((addAnother == 'y' || addAnother == 'Y') && itemsToReturn.size() == 0)
                    {
                        cerr << "At least 1 order item should be returned. Please try again.\n";
                        addAnother = 'n';
                    }
...
{
    string choice;
    Order order(0, 0, {}, "", OrderStatus::Placed, 0); // Create an Order object for menu operations
    ...
    while (true)
    {
        ...
        if (choice == "1")
        {
            ...
            do
            {
                ...
                User customer = User::GetUserById(customerId);

                // Check if the user object is valid (i.e., if the customer exists)
                if (customer.GetId() == 0 || customer.GetId() == -1)
                {
            ...
            do
            {
                ...
                cin >> productCount;
                itemsCountOrdered.push_back(productCount);

                cout << "Add another product? (y/n): ";
                cin >> addAnother;
            } while (addAnother == 'y' || addAnother == 'Y');

            // Calculate total price and apply promotion based on number of items
            double totalPrice = Product::CalculateTotalPrice(itemsOrdered);
            double discountedPrice = Promo::ApplyPromotion(totalPrice, itemsOrdered.size());

            // Create order with discounted price
            order.CreateOrder(customerId, orderDate, OrderStatus::Placed, itemsOrdered, itemsCountOrdered, discountedPrice);
        }
...
void Menu::RunBranchMenu()
{
    ...
    while (true)
    {
        ...
        { // Add Branch
            ...
            Branch branch(0, "", 0);
            branch.AddBranch(location, managerId);
        }
        else if (choice == "2")
        { // Manage Inventory
            int branchId;
...
void Menu::RunInventoryMenu()
{
    ...
    while (true)
    {
        ...
        { // Update Stock Levels
            int productId, newAvailability, newReorderThreshold;
            cout << "Enter product Id: ";
            cin >> productId;
            cout << "Enter new availability: ";
            cin >> newAvailability;
            cout << "Enter new reorder quantity: ";
            cin >> newReorderThreshold;
            inventory.UpdateStockLevels(productId, newAvailability, newReorderThreshold);
        }
        else if (choice == "5")
        {           // Go back
            return; // Exit the loop and return to the previous menu
...
{
    string choice;
    Promo promo(0, "", "", 0.0, "", "", 0, 0); // Create a Promo object for menu operations
    ...
    while (true)
    {
        ...
        { // Create Promotion
            ...
            promo.CreatePromotion(name, description, discountPercentage, startPeriod, endPeriod, productId, employeeId);
        }
        else if (choice == "2")
        { // Apply Promotion
            string promoName;
